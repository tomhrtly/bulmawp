...
function bulmawp_get_breadcrumbs() {
  ...
  if( $breadcrumbsEnable == 1 ) {
		...
		global $post, $wp_query;
		if( !is_front_page() ) {
			echo '<nav class="breadcrumb ', bulmawp_get_breadcrumbs_config(), '" aria-label="breadcrumbs">';
			echo '<ul>';
			echo '<li><a href="', get_home_url(), '">';
			if( $breadcrumbsIcons == 1 ) {
	    	echo '<span class="icon ', bulmawp_breadcrumbs_icon_size(), '"><i class="fas fa-home"></i></span>';
			}
	    echo '<span>Home</span></a></li>';
			if( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
				echo '<li class="is-active"><a href="#" aria-current="page">';
				if( $breadcrumbsIcons == 1 ) {
					echo '<span class="icon ', bulmawp_breadcrumbs_icon_size(), '"><i class="fas ', bulmawp_breadcrumbs_font_awesome_size(), ' fa-calendar"></i></span>';
				}
				...
				echo '<li class="is-active"><a href="#" aria-current="page">';
				if( $breadcrumbsIcons == 1 ) {
					echo '<span class="icon ', bulmawp_breadcrumbs_icon_size(), '"><i class="fas ', bulmawp_breadcrumbs_font_awesome_size(), ' fa-home"></i></span>';
				}
				...
				if( !empty( $last_category ) ) {
					...
					echo '<li class="is-active"><a href="#" aria-current="page">';
					if( $breadcrumbsIcons == 1 ) {
						echo '<span class="icon ', bulmawp_breadcrumbs_icon_size(), '">';
						if( empty( $postIcon) ) {
						} else {
							echo '<i class="fas ', bulmawp_breadcrumbs_font_awesome_size(), ' ', $postIcon, '"></i>';
						}
						echo '</span>';
					}
					echo '<span>', get_the_title(), '</span></a></li>';
				...
				echo '<li class="is-active"><a href="#" aria-current="page">';
				if( $breadcrumbsIcons == 1 ) {
					echo '<span class="icon ', bulmawp_breadcrumbs_icon_size(), '"><i class="fas ', bulmawp_breadcrumbs_font_awesome_size(), ' fa-file"></i></span>';
				}
				...
				echo '<li class="is-active"><a href="#" aria-current="page">';
				if( $breadcrumbsIcons == 1 ) {
					echo '<span class="icon ', bulmawp_breadcrumbs_icon_size(), '"><i class="fas ', bulmawp_breadcrumbs_font_awesome_size(), ' fa-tag"></i></span>';
				}
				...
				if( $breadcrumbsIcons == 1 ) {
					...
					if( empty( $postIcon ) ) {
						...
						echo '<i class="fas ', bulmawp_breadcrumbs_font_awesome_size(), ' fa-search"></i>';
					}
					echo '</span>';
				}
...
/**
 * Displays the BulmaWP dashboard widget within the admin area.
 *
 * @since 0.1
 */
...
function bulmawp_enqueue() {
  ...
  wp_enqueue_script( 'font-awesome', 'https://use.fontawesome.com/releases/v5.5.0/js/all.js', '', '5.3.1', true );
...
function bulmawp_pagination() {
	...
	echo '<nav class="pagination ', bulmawp_get_pagination_config(), '" role="navigation" aria-label="pagination">';
