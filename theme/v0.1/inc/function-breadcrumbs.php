...
function bulmawp_breadcrumbs_alignment() {
  $options = get_option('breadcrumbs_alignment');
	...
	foreach($alignments as $alignment) {
		...
		echo '<label class="radio"><input type="radio" name="breadcrumbs_alignment" value='.$alignment.' '.$checked.'/><code>'.$alignment.'</code></label>';
	}
}
...
function bulmawp_breadcrumbs_alternative_seperators() {
  $options = get_option('breadcrumbs_alternative_seperators');
	...
	foreach($seperators as $seperator) {
		...
		echo '<label class="radio"><input type="radio" name="breadcrumbs_alternative_seperators" value='.$seperator.' '.$checked.'/><code>'.$seperator.'</code></label>';
	}
}
...
function bulmawp_breadcrumbs_icons() {
  $options = get_option('breadcrumbs_icons');
	$checked = ( @$options == 1 ? 'checked' : '' );
	echo '<label class="checkbox"><input type="checkbox" name="breadcrumbs_icons" value="1" '.$checked.' /></label>';
}
...
function bulmawp_breadcrumbs_sizes() {
  $options = get_option('breadcrumbs_sizes');
	...
	foreach($sizes as $size) {
		...
		echo '<label class="radio"><input type="radio" name="breadcrumbs_sizes" value='.$size.' '.$checked.'/><code>'.$size.'</code></label>';
	}
}
?>
